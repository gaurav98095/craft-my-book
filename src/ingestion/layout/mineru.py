...
*understand* them - that's Qwen-VL's job in a later module. Keeping "find" and
"understand" as separate stages is a deliberate separation of responsibilities.
...
def run_mineru(source_path: str | Path, out_dir: str | Path, backend: str = INGESTION.layout.backend) -> Path:
    """
    ...
    if not matches:
        raise RuntimeError(f"mineru produced no _content_list.json for '{source_path.name}' under {out_dir}")
    return matches[-1]
...
def _block_text(block: dict) -> str:
    ...
    what the later Qwen-VL stage (Module 1.4) fills in.
    """
...
def parse_content_list(path: str | Path) -> list[IngestedSegment]:
    ...
    for i, block in enumerate(blocks):
        ...
        extra = {k: v for k, v in block.items() if k not in {"type", "page_idx", "bbox", "text"}}
        segments.append(
            IngestedSegment(
...
class LayoutIngestor(Ingestor):
    ...
    def __init__(self, out_dir: str | Path = INGESTION.layout.work_dir, backend: str = INGESTION.layout.backend):
    ...
    def ingest(self, source_path: str | Path) -> IngestedDocument:
        ...
        page_count = max((s.page for s in segments if s.page is not None), default=-1) + 1
        return IngestedDocument(
            source=Path(source_path).name,
            source_type=self.source_type,
            segments=segments,
            metadata={"page_count": page_count, "mineru_output": str(content_list_path)},
        )
